...
with st.sidebar:
    ...
    st.markdown("### 🔄 Reorder Threshold (RT)")
    ...
    with col5:
        doi_step  = st.number_input("Step",   min_value=1,   max_value=30,  value=1,  step=1)
...
def validate() -> list[str]:
    ...
    if doi_step < 1:
        errors.append("DOI Step must be at least 1.")
...
def write_config(work_dir: str, output_dir: str, csv_path: str) -> str:
    ...
    content = textwrap.dedent(f"""\
        ...
        TARGET_DOI_RANGE        = range({doi_start}, {doi_stop}, {doi_step})
...
n_doi = max(0, len(range(doi_start, doi_stop, doi_step)))
n_scenarios = n_rt * n_doi

c1, c2, c3 = st.columns(3)
c1.metric("RT values to test",  n_rt,        f"RT {rt_start} → {rt_stop - 1}")
c2.metric("DOI values to test", n_doi,       f"DOI {doi_start} → {doi_stop - 1} (step {doi_step})")
...
# ─────────────────────────────────────────────────────────────
    ...
    with st.expander("ℹ️ Parameter Guide"):
        ...
        | **DOI Start / Stop / Step** | Range of Days-of-Inventory tested. Same exclusive Stop rule applies |
        | **Daily SKU Capacity** | Max unique SKUs your warehouse can receive in a single day |
